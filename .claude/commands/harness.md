...
Use a Bash write to avoid stale-file edit problems. Always resolve the path via the repo root, never a bare relative path — a subagent's cwd may resolve to a subdirectory (e.g. `BE/`), and a relative `.harness/runtime/...` write would silently create a stray duplicate file there instead of updating the real one at repo root:

```bash
ROOT="$(git rev-parse --show-toplevel)"
mkdir -p "$ROOT/.harness/runtime"
cat > "$ROOT/.harness/runtime/agent-status.json" <<'JSON'
{
  "main": {
    "command": "harness",
    "skill": "harness"
